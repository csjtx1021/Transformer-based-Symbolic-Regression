...
class NSR():
    def __init__(self, fast_flag=True):
        ...
        self.fitfunc = partial(model.fitfunc_batch,cfg_params=params_fit, fast_flag=fast_flag)
    ...
    def run(self, X, y):
        ...
        print(' \r\n###best preds:', prediction['best_preds'])
        print(' \r\n###best mse:', prediction['best_loss'])
...
if __name__ == "__main__":
    ...
    target_eq = "x_1*sin(x_1) - x_2*x_3" #Use x_1,x_2 and x_3 as independent variables
    #target_eq = "x_1**2 + 2.0*x_1 + 3.0" #Use x_1,x_2 and x_3 as independent variables
    y1 = lambdify(",".join(eq_setting["total_variables"]), target_eq)(**X_dict)
    
    target_eq = "x_1**2 + x_1 * (1 - x_2) + 3.0" #Use x_1,x_2 and x_3 as independent variables
    ...
    prediction = nsr.run(X, torch.cat([y1.unsqueeze(0), y2.unsqueeze(0)], dim=0))
    #prediction = nsr.run(X, torch.cat([y1.unsqueeze(0)], dim=0))
